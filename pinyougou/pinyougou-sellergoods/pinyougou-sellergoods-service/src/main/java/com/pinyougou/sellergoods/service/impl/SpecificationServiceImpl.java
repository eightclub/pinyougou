...
public class SpecificationServiceImpl extends BaseServiceImpl<TbSpecification> implements SpecificationService {
    ...
    public void updateSpecification(Specification specification) {
        ...
        //2. 根据规格id删除其对应的所有规格选项；delete from tb_specification_option where spec_id=?
        TbSpecificationOption param = new TbSpecificationOption();
    ...
    public void deleteSpecificationByIds(Long[] ids) {
        //根据选择了的那些规格id批量删除规格；delete from tb_specification where id in(?,?...)
        deleteByIds(ids);

        //根据选择了的那些规格id批量删除规格选项；delete from tb_specification_option where spec_id in(?,?...)
